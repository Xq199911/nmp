...
def _make_positionless_torch(centroids: "torch.Tensor"):
    ...
    for i in range(0, D - 1, 2):
        ...
        c[:, i] = r
        c[:, i + 1] = torch.zeros_like(c[:, i + 1])
    return c


__all__ = ["reconstruct_with_centroids", "ReconstructedAttention", "ReconstructedAttentionTorch", "augment_kv_with_centroids_torch"]
...
def merge_with_centroids(keys: np.ndarray, values: np.ndarray, centroids: np.ndarray, centroid_weights: np.ndarray, top_k: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    # replicate values for centroids as zeros if none provided (caller may refine)
    if values is None or values.shape[0] == 0:
        dv = centroids.shape[1]
        centroid_values = np.zeros((centroids.shape[0], dv), dtype=np.float32)
    else:
        # naive strategy: project centroid into same value dim by linear mapping is left to caller.
        # Here we simply average existing values as proxy.
        centroid_values = np.tile(values.mean(axis=0, keepdims=True), (centroids.shape[0], 1))

    # scale centroids by sqrt(weight) (heuristic)
