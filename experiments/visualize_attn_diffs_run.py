...
def mean_abs_diffs(on, off):
    layers = sorted(set(list(on.keys()) + list(off.keys())))
    diffs = {}
    for li in layers:
        a = on.get(li)
        b = off.get(li)
        ...
        try:
            ...
            diffs[li] = float(np.mean(np.abs(a2 - b2)))
        except Exception:
            diffs[li] = None
    return diffs


def plot_bar(diffs, out_png):
