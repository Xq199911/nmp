"""
Small validation: generate needles near clusters and run a few recommended configs to check recall.
"""
...
def main():
    ...
    # recommended configs from earlier
    configs = []
    base = {"total_tokens": 4000, "dim": 64, "n_clusters": 20, "cluster_std": 0.5, "n_needles": 20, "batch_size": 32, "flush_interval": 256, "device": device, "agg_similarity": 0.7, "recall_threshold": 0.85, "max_gpu_centroids_per_layer": 512}
    ...
    configs.append({**base, "cluster_kwargs": {"init_preserve_first_n": None, "similarity_threshold": 0.4, "min_merge_similarity": 0.6, "window_size": 50}})
    ...
    out = []
    out_dir = os.path.join("experiments", "needles_near_out")
    os.makedirs(out_dir, exist_ok=True)
    for cfg in configs:
        print("Running:", cfg["cluster_kwargs"])
        res = run_config(cfg)
        out.append(res)
